...
class YOLO {
  ...
  // Tweak these values to get more or fewer predictions.
    //confidenceThreshold: 表示させたい確信度
  let confidenceThreshold: Float = 0.4
  let iouThreshold: Float = 0.5

    //static:構造体=初期値が固定される＆変更も可能だが、初期値はずっと固定
  struct Prediction {
    let classIndex: Int
    let score: Float
    let rect: CGRect
    ...
    //tinyv2...coreML = ~.mlmodelの読み込み
  let model = tinyv2_train20()
  ...
  public func computeBoundingBoxes(features: MLMultiArray) -> [Prediction] {
    assert(features.count == 60*13*13)
    ...
    //Core ML推論モデル内では実行されないため、自分で追加する必要がある。
    //let confidenceThreshold: Float = 0.4 で指定した確率以上の推論...
    //例えば90%の確率と80%の確率のボックスが表示されてしまう。そのため nonMaxSuppression()
    //を使用して複数出ないようにしている。
    //詳しくは "https://apple.github.io/turicreate/docs/userguide/object_detection/export-coreml.html"
